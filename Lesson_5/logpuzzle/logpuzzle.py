...
def read_urls(filename):
    ...
    """
    URL_SRC = r'http://apple-cat.ru'
    data = open(filename).read()
    urls = list(set([URL_SRC + src for src in re.findall(r'\s(/.*_\d+\.jpg)\s', data)]))
    return sorted(urls, key=lambda x: re.findall(r'.*_(\d+)\.jpg$', x)[0])
...
def download_images(img_urls, dest_dir):
    ...
    for index, url in enumerate(img_urls):
        file_path = os.path.join(dest_dir, 'img{}.jpg'.format(index))
        urllib.request.urlretrieve(url, file_path)
    imgs = []
    for src in os.scandir(dest_dir):
        templ = '<img src="{}">'
        imgs.append(templ.format(os.path.abspath(src.path)))
    html = '''<html>
<body>
{}
</body>
</html>'''.format(''.join(imgs))
