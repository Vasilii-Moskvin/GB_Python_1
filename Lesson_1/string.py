...
def not_bad(s):
    index_ne = s.rfind('не')
    index_plox = s.rfind('плох')

    return s[:index_ne] + 'хорош' + s[index_plox + 4:] if index_ne != -1 and\
                                                          index_plox != -1 and\
                                                          index_ne < index_plox\
                                                       else s
...
def front_back(a, b):
    delim_a = int(len(a) / 2) if len(a) % 2 == 0 else int(len(a) / 2) + 1
    delim_b = int(len(b) / 2) if len(b) % 2 == 0 else int(len(b) / 2) + 1
    a1, a2 = a[:delim_a], a[delim_a:]
    b1, b2 = b[:delim_b], b[delim_b:]
    return a1 + b1 + a2 + b2 
